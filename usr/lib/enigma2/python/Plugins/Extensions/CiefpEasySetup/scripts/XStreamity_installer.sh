...
MY_IPK="enigma2-plugin-extensions-xstreamity_5.42_all.ipk"
MY_DEB="enigma2-plugin-extensions-xstreamity_5.42_all.deb"
...
if [ -f $MY_TMP_FILE ]; then
	...
	if [ $MY_RESULT -eq 0 ]; then
		...
		if [ "$DO_REBOOT" = "1" ]; then
			...
			echo "   >>>>   SKIP_REBOOT=1 - MANUAL RESTART REQUIRED   <<<<"
		fi
	else
